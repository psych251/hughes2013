...
Following original protocol of the original authors I plan to analyze collected data with a 2 (test type: memory vs. vocabulary) × 2 (time of report: baseline vs. post-test) Mixed Analysis of Variance to test for main effects and interactions. Additionally, I will use a box-plot and to further quantify visually how memory and vocabulary groups differ given mixed results during Pilot B. 
...
hughes <- read.csv("/Users/home/Desktop/anonymized-results.csv", header = TRUE,  sep=",")
...
Rep.test1 <- aov(age ~ type*condition + Error(ID/condition), data = hughes_gather)

fm1.table <- xtable(Rep.test1)
...
test <- hughes.tidy[order(hughes.tidy$type, hughes.tidy$ID), ]
test.mem <- test[c(1:29),c(1:4)]
test.vocab <- test[c(30:58),c(1:4)]

# t.test for differences in memory condition
t.test(test.mem$Baseline, test.mem$Posttest, paired=TRUE)

# t.test for differences in vocbulary condition
t.test(test.vocab$Baseline, test.vocab$Posttest, paired=TRUE)
...
```{r}
 library(wesanderson)
mean.box<-ggplot(hughes_gather, aes(x=type, y=age, fill=condition)) + geom_boxplot() + 
    theme(axis.line = element_line(colour = "black"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_blank(),
...
mean.box + scale_fill_manual(values=wes_palette(n=2, name="Moonrise2"))

```

```{r}
#remove self-report outliers

explore_gather <-hughes_gather[c(2:12,14:24,27:31,33,35:49,51,53:58,60:70,72:82,85:89,91,93:107,109,111:116),c(1:4)]

explore.mem <- test.mem[c(2:12, 14:25, 27:29),c(1:4)]
explore.vocab <-test.vocab[c(1:5, 7:12,14:16, 19:25, 27:29),c(1:4)]

# t.test for differences in memory condition
t.test(explore.mem$Baseline, explore.mem$Posttest, paired=TRUE)

# t.test for differences in vocbulary condition
t.test(explore.vocab$Baseline, explore.vocab$Posttest, paired=TRUE)

Rep.test2 <- aov(age ~ type*condition + Error(ID/condition), data = explore_gather)

fm2.table <- xtable(Rep.test2)
kable(fm2.table, caption = "Anova table", digits = 3)

# Non-Parametric tests
wilcox.test(explore.mem$Baseline, explore.mem$Posttest, paired=TRUE)
wilcox.test(explore.vocab$Baseline, explore.vocab$Posttest, paired=TRUE)
ks.test(explore.mem$Baseline, explore.mem$Posttest)
ks.test(explore.vocab$Baseline, explore.vocab$Posttest)
...
```{r}
mean.box2<- ggplot(explore_gather, aes(x=type, y=age, fill=condition)) + geom_boxplot() + 
    theme(axis.line = element_line(colour = "black"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_blank(),
...
mean.box2 + scale_fill_manual(values=wes_palette(n=2, name="Moonrise2"))
...
For my replication of study #3 in Hughes, Geraci & De Forrest 2013, I implemented a similar MTurk design using the same Qualtrics memory and vocabulary tests. After data collection of participants (N=58), I organized data and performed a 2 (test type: memory vs. vocabulary) x 2 (time of report: pretest vs. post-test) mixed ANOVA . Previously published results were fully replicated with a significant interaction [F(1, 56) = 6.13, p =0.02], that was driven by a memory-test t(28)=3.26, p=0.003, and not by taking a vocabulary test [ t(28)=0.66, p=0.52]. Therefore, when older adults take a memory test results suggest that subjective age increases relative to a subjective baseline rating.  

### Commentary

Insights from exploratory analysis are important to properly place this replication in a proper context. First, I removed all outliers as inspected through a box plot and looking through how subjects rated themselves.  I once again performed a  2 (test type: memory vs. vocabulary) x 2 (time of report: pretest vs. post-test) mixed ANOVA with results indicating an effect for condition [F(1,48) = 5.08, p=0.03], and no interaction [F(1,48) = 1.35, p=0.25]. Further delineation using T-tests once again confirmed a significant effect for taking a memory test [t(25)=2.68, p=0.01], but not for vocabulary. Subsequent non-parametric testing revealed similar results. 

Using similar test strategies and materials for this replication, results suggest a full replication with some points to consider in future investigations. First, in the original study, no mention was made about removing outliers (e.g. people who rated themselves as 30 years older or younger) and this might have driven previous results as exploratory analysis suggests.
Interestingly, one participant maxed out their subjective age for both conditions at 120 years of age, while a few others would rate themselves under 20 years of age. These skewed subjective ratings lead this author to posit if the slider might be driving effects shown in this replication, and future investigations with a more explicit slider might help answer these questions. Test length could also of factored into this replication study. In the shortened version that I implemented in MTurk little cognitive testing was performed in comparison to the original study which consisted of more cognitive tests. Additional exploratory analysis could look at how performance and varying test length might impact subjective rating. 
